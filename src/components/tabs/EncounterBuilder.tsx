...
export default function EncounterBuilder() {
  const { encounters, upsertEncounter, deleteEncounter, monsterStatblocks, pcs, sessions, lore, locations, overview } = useCampaign();
  ...
  // Auto-select newly created encounters
  useEffect(() => {
    if (!pendingSelectId) return;
    const enc = encounters.find(e => e.id === pendingSelectId);
    if (enc) {
      setSelected(enc);
      setPendingSelectId(null);
    }
  }, [encounters, pendingSelectId]);
  ...
  const openAdd = async () => {
    ...
    await upsertEncounter({
      id: newId,
      name: 'New Encounter',
      description: null, environment: null, difficulty: null,
      party_size: null, party_level: null, combatants: null, dm_notes: null,
      status: 'draft',
...
For each combatant: if it matches a creature in the saved library (same name), set source to "saved", otherwise "custom". Use appropriate CRs for the party level and difficulty. Include 2-4 distinct combatant types for variety.`;
    ...
    try {
      ...
      await upsertEncounter({
        id: newId,
        name: parsed.name ?? 'New Encounter',
        description: parsed.description || null,
        environment: ENVIRONMENTS.includes(parsed.environment) ? parsed.environment : null,
        difficulty: DIFFICULTIES.includes(parsed.difficulty) ? parsed.difficulty : genDifficulty,
        party_size: genPartySize ? parseInt(genPartySize, 10) : null,
        party_level: genPartyLevel ? parseInt(genPartyLevel, 10) : null,
        dm_notes: parsed.dm_notes || null,
        status: 'draft',
        combatants: resolvedCombatants.length > 0 ? JSON.stringify(resolvedCombatants) : null,
        sort_order: encounters.length,
      });
    } catch (err) {
...
// ================================================================
// Encounter edit panel (always-on autosave)
// ================================================================
...
function EncounterEditPanel({
  ...
  const { upsertEncounter, upsertMonsterStatblock } = useCampaign();
  ...
  const [form, setForm] = useState<EncounterForm>({
    ...
    party_size: encounter.party_size != null ? String(encounter.party_size) : '',
    party_level: encounter.party_level != null ? String(encounter.party_level) : '',
    dm_notes: encounter.dm_notes ?? '',
    status: encounter.status,
  });
  const [combatants, setCombatants] = useState<EncounterCombatant[]>(parseCombatants(encounter.combatants));
  ...
  const [genStatblockFor, setGenStatblockFor] = useState<string | null>(null);

  const saveData = useMemo(
    () => ({ ...form, _combatants: JSON.stringify(combatants) }),
    [form, combatants]
  );
  ...
  const { status } = useAutoSave({
    data: saveData,
    onSave: async (data) => {
      const parsed = JSON.parse(data._combatants) as EncounterCombatant[];
      await upsertEncounter({
        id: encounter.id,
        name: data.name.trim() || encounter.name,
        description: data.description || null,
        environment: data.environment || null,
        difficulty: data.difficulty || null,
        party_size: data.party_size ? parseInt(data.party_size, 10) : null,
        party_level: data.party_level ? parseInt(data.party_level, 10) : null,
        dm_notes: data.dm_notes || null,
        status: data.status,
        combatants: parsed.length > 0 ? JSON.stringify(parsed) : null,
        sort_order: encounter.sort_order,
      });
    },
    delay: 800,
  });

  const canRun = form.status !== 'completed' && combatants.length > 0;
  ...
  const genStatblock = async (combatantId: string) => {
    const c = combatants.find(x => x.id === combatantId);
    if (!c || c.statblock_id) return;
    setGenStatblockFor(combatantId);
    try {
  ...
  "dm_notes": "2-3 sentences of DM tactics and encounter tips"
      ...
      if (res.ok && data.text) {
        ...
        const sb = await upsertMonsterStatblock({
          ...
          sort_order: monsterStatblocks.length,
          armor_class: null, ac_descriptor: null, hit_points: null, hit_dice: null,
          speed: null, str: null, dex: null, con: null, int: null, wis: null, cha: null,
          saving_throws: null, skills: null, damage_immunities: null, damage_resistances: null,
          condition_immunities: null, senses: null, languages: null,
        });
        setCombatants(prev => prev.map(x =>
          x.id === combatantId ? { ...x, source: 'saved' as const, statblock_id: sb.id } : x
        ));
      }
    } catch {
      // non-blocking
    } finally {
      setGenStatblockFor(null);
    }
  };

  return (
    <div style={{ maxWidth: '700px' }}>
      <SaveBar status={status} onDelete={onDelete} label="encounter" />
      ...
      {/* ── Meta row ── */}
      <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginTop: '10px', marginBottom: '20px', flexWrap: 'wrap' }}>
        <select className="as-sel" value={form.status} onChange={e => setForm(prev => ({ ...prev, status: e.target.value as EncounterForm['status'] }))}>
          {STATUSES.map(s => <option key={s} value={s}>{s.charAt(0).toUpperCase() + s.slice(1)}</option>)}
        </select>
        <select className="as-sel" value={form.difficulty} onChange={e => setForm(prev => ({ ...prev, difficulty: e.target.value }))}>
          <option value="">— difficulty —</option>
          {DIFFICULTIES.map(d => <option key={d} value={d}>{d.charAt(0).toUpperCase() + d.slice(1)}</option>)}
        </select>
        <select className="as-sel" value={form.environment} onChange={e => setForm(prev => ({ ...prev, environment: e.target.value }))}>
      ...
      {/* ── Party info ── */}
      <div className="as-grid2" style={{ marginBottom: '20px' }}>
        <div>
          <div style={sectionLabel}>Party Size</div>
          <input className="as-inp" type="number" min={1} max={10} value={form.party_size}
            onChange={e => setForm(prev => ({ ...prev, party_size: e.target.value }))}
            placeholder="e.g. 4" />
        </div>
        <div>
          <div style={sectionLabel}>Avg Party Level</div>
          <input className="as-inp" type="number" min={1} max={20} value={form.party_level}
      ...
      <div style={{ marginBottom: '20px' }}>
        ...
        <MarkdownEditor
          ...
          placeholder="Scene-setting description…"
          minHeight="72px"
        />
      </div>
      ...
      <div style={{ marginBottom: '20px' }}>
        ...
        {combatants.length === 0 && (
          <p className="text-xs" style={{ color: '#897f68', marginBottom: '8px' }}>No combatants added yet.</p>
        )}
        {combatants.map(c => {
          ...
          return (
            <div key={c.id} style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                <button onClick={() => setCombatants(prev => prev.map(x => x.id === c.id ? { ...x, count: Math.max(1, x.count - 1) } : x))}
                  style={{ width: '22px', height: '22px', borderRadius: '3px', border: '1px solid #2e2820', background: '#1c1814', color: '#897f68', cursor: 'pointer', fontSize: '0.85rem', lineHeight: 1 }}>−</button>
                <span style={{ minWidth: '18px', textAlign: 'center', color: '#c9a84c', fontWeight: 700, fontFamily: 'var(--mono)', fontSize: '0.8rem' }}>{c.count}</span>
                <button onClick={() => setCombatants(prev => prev.map(x => x.id === c.id ? { ...x, count: x.count + 1 } : x))}
                  style={{ width: '22px', height: '22px', borderRadius: '3px', border: '1px solid #2e2820', background: '#1c1814', color: '#897f68', cursor: 'pointer', fontSize: '0.85rem', lineHeight: 1 }}>+</button>
              </div>
              ...
              <div style={{ minWidth: 0 }}>
                {sb ? (
                  <button onClick={() => onViewStatblock(sb)}
                    style={{ color: '#e8dcc4', fontWeight: 600, fontFamily: 'var(--display)', fontSize: '0.88rem', background: 'none', border: 'none', cursor: 'pointer', padding: 0, textDecoration: 'underline dotted', textUnderlineOffset: '3px', textDecorationColor: '#3e3428', textAlign: 'left' }}>
                    {c.name}
                  </button>
                ) : (
                  <span style={{ color: '#e8dcc4', fontWeight: 600, fontFamily: 'var(--display)', fontSize: '0.88rem' }}>{c.name}</span>
                )}
                <div style={{ color: '#897f68', fontSize: '0.68rem', fontFamily: 'var(--mono)', marginTop: '1px' }}>
                  {[c.challenge_rating ? `CR ${c.challenge_rating}` : null, c.creature_type].filter(Boolean).join(' · ')}
                  {!sb && c.source === 'custom' && (
                    <button onClick={() => genStatblock(c.id)} disabled={genStatblockFor === c.id}
                      style={{ marginLeft: '8px', color: genStatblockFor === c.id ? '#897f68' : '#c9a84c', background: 'none', border: 'none', cursor: 'pointer', fontSize: '0.65rem', fontFamily: 'var(--mono)', padding: 0 }}>
                      {genStatblockFor === c.id ? '…' : '✦ Gen stat block'}
                    </button>
                  )}
                </div>
              </div>
              ...
              {/* Remove */}
              <button onClick={() => setCombatants(prev => prev.filter(x => x.id !== c.id))}
                style={{ color: '#4a4438', fontSize: '0.75rem', background: 'none', border: 'none', cursor: 'pointer', padding: '2px 4px' }}>
                ✕
              </button>
            </div>
          );
